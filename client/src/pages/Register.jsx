...
const RegisterModal = ({ open, onClose, onLoginLink }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="relative w-full max-w-xs sm:max-w-sm bg-white/90 rounded-2xl shadow-2xl p-8 border border-[#3b3bb3] animate-fade-in">
        ...
        <h2 className="text-3xl font-extrabold text-[#3b3bb3] mb-6 text-center tracking-tight drop-shadow">Register</h2>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="relative">
            <input type="text" className="w-full border-2 border-[#6366f1] rounded-lg px-8 py-2 focus:outline-none focus:ring-2 focus:ring-[#3b3bb3] bg-white text-gray-900 placeholder-gray-700 transition" value={name} onChange={e => setName(e.target.value)} required placeholder="Name" />
            <FaUser className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-700" />
          </div>
          <div className="relative">
          ...
          <button type="submit" className="bg-gradient-to-r from-[#6366f1] to-[#3b3bb3] text-white px-6 py-3 rounded-xl w-full font-bold text-lg shadow-lg hover:from-[#3b3bb3] hover:to-[#23237a] transition-all mt-2">Register</button>
        </form>
        <div className="mt-4 text-center text-gray-900 text-sm">
          Already have an account?{' '}
          <span className="text-blue-900 font-semibold cursor-pointer hover:underline" onClick={onLoginLink}>Login</span>
        </div>
      </div>
      <style>{`
        .animate-fade-in {
